...
def preprocessing (audio, feature):
    if feature == 'mfcc':
        mfcc = []
        ...
        if X_trim[0].shape[0] >= 50000:
            ...
            zero_padding = tf.zeros([50000]-tf.shape(X_trim[0]),dtype=tf.float32)
            X_final = tf.concat([X_trim[0],zero_padding],0).numpy()
        S = librosa.feature.tonnetz(y=X_final, sr=sr)
        tonnetz = np.stack((S[0],S[1],S[2],S[3],S[4],S[5]))
        X_flat= tonnetz
    return X_flat

def extract_ravdess(feature, mels=256, trim=False, path=path):
    '''
    ...
    for i in np.arange(0, len(speech)):
        ...
        result.append(preprocessing(speech[i],feature=feature))
...
def extract_crema(feature, mels=256, trim=False, path=path):
    '''
    ...
    for i in np.arange(0, len(speech)):
        temp = preprocessing(speech[i],feature=feature)
...
def extract_savee(feature, mels=256, trim=False, path=path):
    '''
...
def extract_tess(feature, mels=256, trim=False, path=path):
    '''
    Extracts audio features from savee db using methods defined in the
    Silvertone object
    '''
...
def extract_ESD(feature, mels=256, trim=False, path=path, eng_only=True):
    '''
    Extracts audio features from ravdess db using methods defined in the
    Silvertone object
    '''
...
def extract_JL(feature, mels=256, trim=False, path=path):
    '''
    Extracts audio features from ravdess db using methods defined in the
    Silvertone object
    '''
...
def extract_ASVP(feature, mels=256, trim=False, path=path):
    '''
    Extracts audio features from ravdess db using methods defined in the
    Silvertone object
    '''
